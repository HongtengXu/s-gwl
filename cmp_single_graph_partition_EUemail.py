"""
Compare our Gromov-Wasserstein graph partitioning method (GWL) with the state-of-the-art methods
...
methods = ['metis', 'FastGreedy', 'Louvain', 'Fluid', 'GWL']
num_nodes = 1005
num_partitions = 42
...
cost = 0.5*database['cost']/num_nodes + 0.5*database['cost'].T/num_nodes
p_s = np.zeros((num_nodes, 1))
p_s[:, 0] = np.sum(np.asarray(cost.todense()), axis=1) ** 0.001
p_s /= np.sum(p_s)
...
with open('results/cmp_single_graph_partition_EUemail.pkl', 'wb') as f:
